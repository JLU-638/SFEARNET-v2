...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for  image_A, image_B, label,edge,img_id in test_bar:
            ...
            #compute OA by own
            label_pred1_O = label_pred1.squeeze().cpu().numpy()
            label = label.squeeze().cpu().numpy()
            # 计算 TP / FP / FN
            TP = np.sum((label_pred1_O == 1) & (label == 1))
            FP = np.sum((label_pred1_O == 1) & (label == 0))
            FN = np.sum((label_pred1_O == 0) & (label == 1))
        ...
        F1_result.sort(key=lambda x:x['sample'], reverse=False)
        print(F1_result)
        for i in F1_result:
            re.append([i["img_id"], i["sample"]])

        data = pd.DataFrame(data=re, index=None, columns=['img_id', 'F1'])
        # #print(data)

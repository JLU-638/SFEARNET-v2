...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for  image_A, image_B, label,edge,img_id in test_bar:
            ...
            correct = (label_pred1_O == label).sum()
            total = label_pred1_O.size
            sample_OA = correct / total
            OA_result.append({
                "img_id":img_id_str,
                "sample":sample_OA
                })
        ...
        OA_result.sort(key=lambda x:x['sample'], reverse=False)
        print(OA_result)
        for i in OA_result:
            re.append([i["img_id"], i["sample"]])

        data = pd.DataFrame(data=re, index=None, columns=['img_id', 'OA'])
        # #print(data)

...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        self.conv1 = nn.Conv2d(
            2, 1, kernel_size, padding=padding, bias=False)  # 7,3     3,1
...
class token_encoder(nn.Module):
    def __init__(self, in_chan=32, token_len=4, heads=8):
        ...
        self.transformer = Transformer(
            dim=in_chan, depth=1, heads=heads, dim_head=64, mlp_dim=64, dropout=0)
...
class token_decoder(nn.Module):
    def __init__(self, in_chan=32, size=32, heads=8):
        ...
        self.pos_embedding_decoder = nn.Parameter(
            torch.randn(1, in_chan, size, size))
        self.transformer_decoder = TransformerDecoder(
            dim=in_chan, depth=1, heads=heads, dim_head=True, mlp_dim=in_chan*2, dropout=0, softmax=in_chan)
...
class Classifier(nn.Module):
    def __init__(self, in_chan=32, n_class=2):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(in_chan * 2, in_chan, kernel_size=3,
...
class Conv_stride(nn.Module):
    def __init__(self, in_channels=32):
        ...
        self.conv = nn.Conv2d(in_channels=32, out_channels=32,
...
class Upsample(nn.Module):
    def __init__(self, scale_factor=2):
        ...
        self.upsample = nn.Upsample(
            scale_factor=scale_factor, mode="bicubic", align_corners=True)
...
class CDNet(nn.Module):
    def __init__(self,  backbone='resnet50', output_stride=16, img_size=512, img_chan=3, chan_num=32, n_class=2):
        ...
        self.backbone = build_backbone(
            backbone, output_stride, BatchNorm, img_chan)
        ...
        self.conv_s8 = nn.Conv2d(chan_num*2, chan_num,
                                 kernel_size=3, padding=1)
        self.conv_s4 = nn.Conv2d(chan_num*3, chan_num,
                                 kernel_size=3, padding=1)
    ...
    def forward(self, img1: Tensor, img2: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        x16 = F.interpolate(
            x16, size=img1.shape[2:], mode='bicubic', align_corners=True)
        x8 = F.interpolate(
            x8, size=img1.shape[2:], mode='bicubic', align_corners=True)
        x = F.interpolate(
            x, size=img1.shape[2:], mode='bicubic', align_corners=True)

...
def val(mloss):
    ...
    if test_loss > mloss or epoch == 1:
        torch.save(CDNet.state_dict(), args.model_dir +
                   '_' + str(test_loss) + '_best.pth')
    CDNet.train()
    return test_loss
...
def train_epoch():
    ...
    for hr_img1, hr_img2, label in train_bar:
        ...
        CD_loss = CDcriterionCD(
            result1, label) + CDcriterionCD(result2, label) + CDcriterionCD(result3, label)
...
if __name__ == '__main__':
    ...
    train_set = DA_DatasetFromFolder(
        args.hr1_train, args.hr2_train, args.lab_train, crop=False)
    test_set = TestDatasetFromFolder(
        args, args.hr1_test, args.hr2_test, args.lab_test)

    train_loader = DataLoader(
        dataset=train_set, num_workers=args.num_workers, batch_size=args.batchsize, shuffle=True)
    test_loader = DataLoader(dataset=test_set, num_workers=args.num_workers,
                             batch_size=args.val_batchsize, shuffle=True)
    ...
    optimizer = optim.AdamW(itertools.chain(
        CDNet.parameters()), lr=args.lr, betas=(0.9, 0.999), weight_decay=0.01)
